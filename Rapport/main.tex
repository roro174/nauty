...
\rodolphe{Je devrais parler de graph6 quelque part non?}

In this section, we will focus extensively on McKay's general algorithm: Nauty.

First, we will examine the creation of \hyperref[def:canonical form]{the canonical form} of a graph in this algorithm from a theoretical perspective. Then, we will explain the algorithm itself by presenting the different implementation strategies.

\section{The Canonical Form}

First of all, the algorithm itself is built upon a very specific type of tree: a search tree \rodolphe{déf de search tree au dessus}.  
...
The root of this tree is therefore the ``empty'' sequence; a sequence in which no vertex has a unique colour assigned by our algorithm.\rodolphe{Je dois retrwailler cette phrase parce qu'elle n'est pas très clair}
...
These three functions are the fundamental components required to construct our search tree and find our canonical form. However, there exist additional functions that are more specific to the implementation of the algorithm and that will be explained in the implementation section of this document. 
...
Before discussing the implementation of the algorithm, we will therefore explain these fundamental functions in more detail and describe how they are used to construct our search tree.
