...
\begin{definition}
    ...
    Let $X$ be a finite set. The \emph{symmetric group} on $X$ is the group of all \emph{permutations} of $X$, i.e.\ all bijections from $X$ to itself, with function composition as the group operation. In particular, when $X=\{1,\ldots,n\}$, this group is denoted by $S_n$.
...
\begin{definition}\label{def:pointwise stabiliser}
  Let a group $G$ act on a set $X$, and let $S \subseteq X$.  
  The \emph{point-wise stabiliser} of $S$ in $G$ is the subgroup defined by:
  \[
...
The \emph{star graph} $K_{1,3}$ is a tree with one internal node and three leaves. It is also known as the \emph{claw graph}. The internal node is called the \emph{centre} of the star graph. For most of our examples, we will use this graph.

\begin{figure}[ht]
    \centering
        \input{figTex/K1_3.tex}
    \caption{The star graph $K_{1,3}$ with its centre and leaves.}
\end{figure}
\end{definition}
...
Informally, the canonical form $C(G,\pi)$ of a coloured graph is a unique representative of the isomorphism class of $(G,\pi)$. In other words, $C$ maps any two 
coloured graphs related by a relabelling to exactly the same output. Consequently, two coloured graphs are isomorphic if and only if they have the same canonical form.
...
The \emph{Polynomial-Time Hierarchy} (\PH) is a hierarchy of complexity classes that generalises the classes \NP{} and \coNP. Formally, the base level of the hierarchy is defined as:
\[
