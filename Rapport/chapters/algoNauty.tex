...
\begin{figure}[htbp]
  \centering
  \resizebox{\linewidth}{!}{%
  \input{figTex/nauty_tree.tex}
  }

\caption{Individualized vertices are represented by double circles. The encircled set in each partition denotes the current target cell.}\label{fig:nauty-tree-k13}
...
\begin{proof}
  To prove the first point, let $v_1$ denote the lexicographically smallest leaf satisfying $\phi(G,\pi_0,v_1)=\phi^*(G,\pi_0)$.
  Operation $P_A$ only removes branches whose invariant cannot reach the maximal value, while operation $P_C$ always preserves the lexicographically smallest representative of an automorphism class. Consequently, $v_1$ cannot be removed. Therefore, at least one leaf with maximal invariant always remains after any sequence of pruning operations.
  
  For the second point, let $g\in\mathrm{Aut}(G,\pi_0)$ and consider the corresponding leaf $v_0^g$.
  
  If $v_0^g$ is still present after pruning, then by definition $g\in A$.
  
  Otherwise, $v_0^g$ must have been removed by an operation $P_C(v'',g_i)$. It cannot be removed by $P_B(v', v)$ because it only removes leaves inequivalent to $v_0$. This implies that there exists a lexicographically smaller equivalent leaf, namely
  \[
  v_0^{gg_i^{-1}}<v_0^g.
  \]
  ...
  such that $v_0^{gh}$ is a remaining leaf. By definition of $A$, we have $gh\in A$, and therefore $ g=(gh)h^{-1}$ where $gh\in A$ and $h\in\langle g_1,\ldots,g_k\rangle$.
...
\begin{algorithmic}[1]
  ...
  \Procedure{InsertGenerator}{$g$, level $i$}
      ...
      \State\Call{CloseStabilizer}{$i$} \Comment{Schreier generators, recurses if needed}
  ...
  This randomization trades a deterministic guarantee for speed: the
...
To demonstrate how $P_C$ works in practice, we return to our search tree for \hyperref[fig:nauty-tree-k13]{$K_{1,3}$} and apply the $P_C$ pruning strategy. As shown, the search tree is significantly reduced due to the first discovered automorphism, $(2,3)$. Subsequently, upon entering the middle branch, another automorphism, $(1,3)$, is identified, enabling us to prune a leaf within that branch. 

  \begin{figure}[htbp]
    \centering
    \resizebox{\linewidth}{!}{%
    \input{figTex/nauty_tree_prunned.tex}
    }
  
  \caption{Search tree for $K_{1,3}$ with the $P_C$ pruning strategy applied.}\label{fig:nauty-tree-k13-prunned}
...
\rodolphe{Tu crois qu'il y a un problème de convention ici et que je devrais juste mettre des $\pi_0$ partout?}
