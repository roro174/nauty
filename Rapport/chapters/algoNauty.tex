...
graph $(G, \pi_0)$. Here, we allow ourselves an abuse of notation, using the refinement function itself to 
denote the permutation associated with the leaf $\sigma^*$. Indeed, since this leaf is 
discrete, it corresponds to an ordered partition of $V(G)$ into singletons (an ordering of the vertices); as the vertex set is already identified with $\{1,\ldots,n\}$, 
this ordering genuinely defines a permutation, and thus an element of $S_n$.
...
\begin{definition}\label{def:refinement_algo}
  ...
  \begin{itemize}
    ...
    \end{itemize}
      \[
      R(G,\pi_0,(v_1,\ldots,v_k)) = F\Big(G,\ I\big(R(G,\pi_0,(v_1,\ldots,v_{k-1})),\ v_k\big),\ \{v_k\}\Big).
      \]


The algorithm satisfies the required properties of a refinement function and performs well in practice. However, the refinement step is the most computationally expensive part of the algorithm, which is why special care must be taken in its implementation. Therefore, to reduce the overall running time, we will employ pruning techniques, which will be discussed later.

\rodolphe{Faire preuve que ça satisfies les propriétés de la fonction de raffinement.}
...
\begin{algorithmic}[1]
    ...
    \ForAll{cells $C_i \in \pi$}
        \State$\mathit{count}(v) \gets 0$ for all vertices $v \in C_i$
        ...
        \ForAll{triples of vertices $\{u,v,w\} \subseteq C_i$}
            ...
            \State$\mathit{count}(u) \gets\mathit{count}(u)+weight$
            \State$\mathit{count}(v) \gets \mathit{count}(v)+weight$
            \State$\mathit{count}(w) \gets \mathit{count}(w)+weight$
        \EndFor{}
        
        \State$I[i] \gets \sum_{v \in C_i} \mathit{count}(v)$
    \EndFor{}
...
\rodolphe{Plus parler du résultat}
