\begin{tikzpicture}[
  ...
  side/.style={align=left, font=\scriptsize, inner sep=2pt},
  >=latex
]

% --- Niveau 0 : G^{(0)} = Aut(K_{1,3}), |G^{(0)}|=6 ---
\node[level0] (root) at (0,7.3)
  {$G^{(0)} = \mathrm{Aut}(K_{1,3})$\\ $|G^{(0)}|=6$\\
   base point $b_1=1$\\ orbit $1^{G^{(0)}}=\{1,2,3\}$};

% --- Niveau 1 : trois cosets de G^{(1)} ---
\node[level1] (A) at (-5.5,4.0) {$\sigma^{(0)}_1=\mathrm{id}$};
\node[level1] (B) at (0,4.0)    {$\sigma^{(0)}_2=(1\,2)$};
\node[level1] (C) at (5.5,4.0)  {$\sigma^{(0)}_3=(1\,3)$};
...
% annotation de niveau 1 : juste à droite du nœud C
\node[side, anchor=west, text width=3cm] at (7.1,4.0)
{$G^{(1)}=\mathrm{Stab}(1)=\{\mathrm{id},(2\,3)\}$, $|G^{(1)}|=2$\\
 base point $b_2=2$, orbit $\{2,3\}$};

% --- Niveau 2 : les feuilles ---
\node[leaf] (A1) at (-7,0.2)   {$g=\mathrm{id}$};
\node[leaf] (A2) at (-4.2,0.2) {$g=(2\,3)$};
\node[leaf] (B1) at (-1.6,0.2) {$g=(1\,2)$};
\node[leaf] (B2) at (1.6,0.2)  {$g=(1\,2\,3)$};
\node[leaf] (C1) at (4.2,0.2)  {$g=(1\,3)$};
\node[leaf] (C2) at (7,0.2)    {$g=(1\,3\,2)$};
...
% annotation de niveau 2 : juste à gauche du nœud A1, symétrique de la précédente
\node[side, anchor=east, text width=2cm] at (-8.4,0.2)
{$G^{(2)}=\{e\}$};
